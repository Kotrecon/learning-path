...
- **Заметки/Наблюдения:**
  ...
  - 💡 Инсайт: Документация = контракт между разработчиком и будущим собой/командой. Чёткие шаги воспроизведения экономят часы отладки.

---

## ✅ Критерий приёмки модуля

- [x] Program.cs использует top-level statements (нет `class Program`, `static void Main`)
- [x] `Host.CreateDefaultBuilder(args).Build()` инициализирует хост
